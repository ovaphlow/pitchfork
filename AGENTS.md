...
- **计划者**：编写和维护实施计划、范围、验收口径和角色矩阵；不以计划之名直接实现或宣告验收通过。
- **实现者**：实现生产代码、Shared API 和 UI，编写并运行计划指定的单元和嵌入式路由测试，执行默认的不访问数据库验证；只修复经验证者或审查者报告的生产缺陷。
- **独立验证者**：编写或维护计划指定的集成/E2E 测试及 fixture，在获授权的隔离环境运行真实数据库和浏览器验收；不得直接修改生产代码、Shared API、UI、单元测试或路由测试。
- **审查者**：独立审查实现 diff、测试设计、验证证据和残余风险，确认计划口径、无副作用和修复闭环；不代替实现者修生产代码，也不把构建通过当作最终验收。只有计划指定的审查者可以记录最终验收结论。
...
- 所有交接必须附修改文件、实际执行命令及结果。独立验证者的失败报告必须提供前置数据、最小复现命令、预期结果、实际结果和脱敏证据；实现者修复后必须重新交付候选版本，验证者只复验受影响项，审查者再作最终判断。
