...
export default function NursingPage() {
  ...
  const assessmentColumns: Column<NursingAssessment>[] = [
    ...
    { key: "assessor", header: "评估人", className: "min-w-[100px]", render: (row) => row.assessor || "-" },
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-fg-muted">围绕当前入住长者完成评估、计划制定、任务安排和执行记录</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState icon="🏠" title="暂无活动入住" description="请先在入住管理为长者办理入住，再开始建立照护记录。" />
      ) : (
        <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
          <Card title="当前入住长者" actions={<span className="text-xs text-fg-dimmed">共 {admissions.length} 人</span>}>
              ...
              <>
                ...
                {activeTab === "overview" && (
                  <div className="grid gap-6 lg:grid-cols-2">
                    <Card title="最近评估">
                      <Table columns={assessmentColumns.slice(0, 5)} data={assessments.slice(0, 5)} loading={false} emptyMessage="暂无评估记录" />
                    </Card>
                    <Card title="当前照护计划">
                ...
                {activeTab === "assessments" && <Card title="护理评估记录" actions={<Button size="sm" onClick={openAssessment}>新增评估</Button>}><Table columns={assessmentColumns} data={assessments} loading={false} emptyMessage="暂无护理评估，建议先完成入住后的 Barthel 指数和风险评估。" /></Card>}
      ...
      <Modal open={assessmentOpen} onClose={() => !saving && setAssessmentOpen(false)} title="新增护理评估">
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void handleCreateAssessment(); }}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Input label="评估人" value={assessmentForm.assessor} onChange={(event) => setAssessmentForm((current) => ({ ...current, assessor: event.target.value }))} placeholder="请输入评估人" />
      ...
      <Modal open={planOpen} onClose={() => !saving && setPlanOpen(false)} title="制定照护计划">
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void handleCreatePlan(); }}>
          <div className="grid gap-4 sm:grid-cols-2"><Input label="计划名称" value={planForm.planName} onChange={(event) => setPlanForm((current) => ({ ...current, planName: event.target.value }))} placeholder="例如：日常生活照护计划" required /><Input label="制定人" value={planForm.createdBy} onChange={(event) => setPlanForm((current) => ({ ...current, createdBy: event.target.value }))} placeholder="请输入制定人" /><Input label="开始日期" type="date" value={planForm.startDate} onChange={(event) => setPlanForm((current) => ({ ...current, startDate: event.target.value }))} /><Input label="结束日期" type="date" value={planForm.endDate} onChange={(event) => setPlanForm((current) => ({ ...current, endDate: event.target.value }))} /></div>
      ...
      <Modal open={executionTask !== null} onClose={() => !saving && setExecutionTask(null)} title="登记任务完成">
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void handleCreateExecution(); }}>
          ...
          <Input label="执行人" value={executionForm.executor} onChange={(event) => setExecutionForm((current) => ({ ...current, executor: event.target.value }))} placeholder="请输入护理员/执行人" />
