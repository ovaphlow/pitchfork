...
object RequisitionRoutes {
    ...
    fun create(vertx: Vertx, pool: Pool): Router {
        ...
        val service = RequisitionService(pool)
        ...
        router.put("/:id/approve").handler { ctx ->
            val id = ctx.pathParam("id") ?: return@handler PharmacyRoutes.respond(ctx, 400, "id required")
            val b = PharmacyRoutes.body(ctx)
            val items = b.getJsonArray("items") ?: JsonArray()
            if (items.isEmpty) {
                PharmacyRoutes.respond(ctx, 400, "items with approved_quantity is required"); return@handler
            }
            service.approve(id, items)
                .onSuccess { ctx.json(it) }
                .onFailure {
                    when (it) {
        ...
        router.put("/:id/dispense").handler { ctx ->
            val id = ctx.pathParam("id") ?: return@handler PharmacyRoutes.respond(ctx, 400, "id required")
            val b = PharmacyRoutes.body(ctx)
            val items = b.getJsonArray("items") ?: JsonArray()
            if (items.isEmpty) {
                PharmacyRoutes.respond(ctx, 400, "items with dispensed_quantity is required"); return@handler
            }
            service.dispense(id, items)
                .onSuccess { ctx.json(it) }
                .onFailure {
                    when (it) {
                        is NotFoundException -> PharmacyRoutes.respond(ctx, 404, it.message)
