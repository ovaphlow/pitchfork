...
class TaskExecutionService(
    ...
    private fun executionWithSummaryJson(row: Row, now: OffsetDateTime = OffsetDateTime.now()): JsonObject {
        val json = toJson(row)
        ...
        val plannedTime = row.getValue("planned_time")?.let {
            if (it is OffsetDateTime) it
            else if (it is String) try { OffsetDateTime.parse(it) } catch (_: Exception) { null }
            else null
        }
    ...
    companion object {
        ...
        fun computeOverdueFields(
            ...
            val isOverdue = (status == "PENDING" || status == "IN_PROGRESS")
    ...
    fun updateStatus(id: String, newStatus: String, note: String? = null): Future<JsonObject> {
    ...
    fun completeExecutionWithConsumptions(
        ...
        return get(id).flatMap { existing ->
            ...
            pool.withTransaction { connection ->
                ...
                connection.preparedQuery(DatabaseConfig.sql(lockExec))
                    ...
                    .compose { lockedRows ->
                                ...
                                .compose { existingResult ->
                                    ...
                                    if (existingConsumptions == null || existingConsumptions.size() == 0) {
                                        ...
                                        return@compose Future.failedFuture(
                                            ConflictException("execution already completed without consumptions, cannot add consumptions now")
                                        )
                                    }
                                    // 比较耗材清单 — 按单位类型规范化 quantity
                                    val existingSummary = existingConsumptions.map {
                                        val obj = it as JsonObject
                                        val unit = obj.getString("unit") ?: "PACKAGE"
                                        val qty = if (unit == "SPLIT") obj.getDouble("split_quantity") ?: 0.0
                                                  else obj.getDouble("quantity") ?: 0.0
                                        obj.getString("stock_id") to (unit to qty)
                                    }.toSet()
                                    val newSummary = consumptions.map {
                                        val unit = it.unit
                                        val qty = if (unit == "SPLIT") it.splitQuantity?.toDouble() ?: 0.0
                                                  else it.quantity?.toDouble() ?: 0.0
                        ...
                        // 查询任务和周期信息
                        val taskInfo = ctx.select(ctPeriodId)
                            .from(taskTable)
                            .where(DSL.field("t.id").eq(taskId))
                        connection.preparedQuery(DatabaseConfig.sql(taskInfo))
                            .execute(DatabaseConfig.tuple(taskInfo))
                            .compose { taskRows ->
                                val periodId = if (taskRows.size() > 0) taskRows.iterator().next().getValue("t_period_id")?.toString() else null

                                val periodQuery = ctx.select(cpPatientId).from(periodTable).where(DSL.field("p.id").eq(periodId))
                                connection.preparedQuery(DatabaseConfig.sql(periodQuery))
                                    .execute(DatabaseConfig.tuple(periodQuery))
                                    .compose { periodRows ->
                                        val patientId: String = if (periodRows.size() > 0) periodRows.iterator().next().getValue("p_patient_id")?.toString() ?: "" else ""
    ...
    /** 加载已有完成的执行及其耗材 */
    private fun loadExistingExecutionWithConsumption(connection: SqlConnection, id: String): Future<JsonObject> {
        val readQuery = ctx.selectFrom(t).where(cId.eq(id))
        return connection.preparedQuery(DatabaseConfig.sql(readQuery))
            ...
            .compose { rows ->
                ...
                connection.preparedQuery(DatabaseConfig.sql(consQuery))
                    ...
                    .map { consRows ->
                        ...
                        record.put("consumption_summary", JsonObject()
    ...
    private fun insertConsumptions(
        ...
        fun process(index: Int): Future<Void?> {
            ...
            val insertQ = ctx.insertInto(consTable)
            ...
            return connection.preparedQuery(DatabaseConfig.sql(insertQ))
    ...
    fun ensureExecutionsForDateRange(
        ...
        return pool.preparedQuery(DatabaseConfig.sql(taskQuery))
            ...
            .flatMap { taskRows ->
                ...
                for (taskRow in taskRows) {
                    ...
                    val metadata = taskRow.getValue("t_metadata")?.let {
                        when (it) {
                            is JsonObject -> it
                            is String -> try { JsonObject(it) } catch (_: Exception) { null }
                            else -> null
                        }
                    }
                    ...
                    while (!date.isAfter(dateTo)) {
                        if (endDate != null && date.isAfter(endDate)) { date = date.plusDays(1); continue }
    ...
    fun ensureExecutionsForDate(
        ...
        return ensureExecutionsForDateRange(date, date, periodId)
    }
    ...
    fun todayExecutions(
        ...
        return ensureExecutionsForDate(date, periodId).flatMap {
            ...
            val baseSelect = ctx.select(allColumns)
            ...
            val overdueTotalQuery = ctx.select(count().`as`("overdue_total"))
                .from(DSL.table(DSL.name("nursing", "nursing_task_executions")).`as`("e"))
                .join(DSL.table(DSL.name("nursing", "nursing_tasks")).`as`("t"))
                .on(DSL.field("e.task_id").eq(DSL.field("t.id")))
                .leftJoin(DSL.table(DSL.name("nursing", "nursing_service_periods")).`as`("p"))
            ...
            val dataQuery = baseSelect
                .orderBy(DSL.field("e.planned_time").asc())
                .limit(limit)
                .offset(offset)

            pool.preparedQuery(DatabaseConfig.sql(countQuery))
                .execute(DatabaseConfig.tuple(countQuery))
                .flatMap { countRows ->
                    val total = countRows.iterator().next().getLong("total") ?: 0L
                    pool.preparedQuery(DatabaseConfig.sql(dataQuery))
                        ...
                        .flatMap { dataRows ->
                            ...
                            pool.preparedQuery(DatabaseConfig.sql(overdueTotalQuery))
